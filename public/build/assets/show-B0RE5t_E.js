let i=null;function o(){const t=document.querySelector("[data-shop-slug]");return t?t.dataset.shopSlug:null}function p(){const t=document.querySelectorAll(".tab-button"),e=document.querySelectorAll(".tab-content");t.forEach(a=>{a.addEventListener("click",()=>{const s=a.dataset.tab;t.forEach(n=>{n.classList.remove("active","border-blue-500","text-blue-600"),n.classList.add("border-transparent","text-gray-500")}),a.classList.remove("border-transparent","text-gray-500"),a.classList.add("active","border-blue-500","text-blue-600"),e.forEach(n=>{n.classList.add("hidden")}),document.getElementById(`tab-${s}`).classList.remove("hidden")})})}async function b(){var e,a;const t=o();if(!t){console.error("Shop slug not found");return}try{i=(await axios.get(`/api/admin/shops/${t}`)).data,h(i)}catch(s){console.error("Error loading shop:",s),document.getElementById("loading").classList.add("hidden");const n=document.getElementById("error");n.classList.remove("hidden"),n.querySelector("p").textContent=((a=(e=s.response)==null?void 0:e.data)==null?void 0:a.message)||"Failed to load shop details"}}function h(t){document.getElementById("loading").classList.add("hidden"),document.getElementById("shop-details").classList.remove("hidden");const e=t.deleted_at!==null;document.getElementById("shop-header").innerHTML=`
    ...
    `;const s=["monday","tuesday","wednesday","thursday","friday","saturday","sunday"].map(r=>{const g=t[`${r}_closed`];let l="Closed";return!g&&t[`${r}_open`]&&t[`${r}_close`]?l=v(t[`${r}_open`])+" - "+v(t[`${r}_close`]):g?l="Closed":l="Not set",`
            <div>
                <h3 class="text-sm font-medium text-gray-500 mb-2">${r.charAt(0).toUpperCase()+r.slice(1)}</h3>
                <p class="text-lg text-gray-900">${l}</p>
            </div>
        ...
        `),document.getElementById("shop-actions").innerHTML=n.join(""),f(e)}function f(t){if(t){const e=document.getElementById("restore-btn"),a=document.getElementById("permanent-delete-btn");e&&e.addEventListener("click",$),a&&a.addEventListener("click",m)}else{const e=document.getElementById("toggle-status-btn"),a=document.getElementById("archive-btn");e&&e.addEventListener("click",d),a&&a.addEventListener("click",c)}}function d(t){const a=!i.is_active?"activate":"deactivate";window.toast.warning(`Click again to ${a} shop`,3e3);const s=t.target,n=s.textContent;s.textContent=`Confirm ${a.charAt(0).toUpperCase()+a.slice(1)}`,s.removeEventListener("click",d),s.addEventListener("click",u,{once:!0}),setTimeout(()=>{s.textContent=n,s.removeEventListener("click",u),s.addEventListener("click",d)},3e3)}async function u(){var s,n;const t=o();if(!t||!i)return;const e=!i.is_active,a=e?"activate":"deactivate";try{await axios.patch(`/api/admin/shops/${t}`,{name:i.name,slug:i.slug,phone:i.phone,email:i.email,is_active:e}),window.toast.success(`Shop ${a}d successfully!`),setTimeout(()=>window.location.reload(),1e3)}catch(r){console.error("Error updating shop status:",r),window.toast.error(((n=(s=r.response)==null?void 0:s.data)==null?void 0:n.message)||"Failed to update shop status")}}function c(t){window.toast.warning("Click Archive again to confirm",3e3);const e=t.target;e.textContent="Confirm Archive",e.removeEventListener("click",c),e.addEventListener("click",x,{once:!0}),setTimeout(()=>{e.textContent="Archive Shop",e.removeEventListener("click",x),e.addEventListener("click",c)},3e3)}async function x(){var e,a;const t=o();if(t)try{await axios.delete(`/api/admin/shops/${t}`),window.toast.success("Shop archived successfully!"),setTimeout(()=>window.location.reload(),1e3)}catch(s){console.error("Error archiving shop:",s),window.toast.error(((a=(e=s.response)==null?void 0:e.data)==null?void 0:a.message)||"Failed to archive shop")}}async function $(){var e,a;const t=o();if(t)try{await axios.post(`/api/admin/shops/${t}/restore`),window.toast.success("Shop restored successfully!"),setTimeout(()=>window.location.reload(),1e3)}catch(s){console.error("Error restoring shop:",s),window.toast.error(((a=(e=s.response)==null?void 0:e.data)==null?void 0:a.message)||"Failed to restore shop")}}function m(t){window.toast.warning("Click Delete again to permanently delete",3e3);const e=t.target;e.textContent="Confirm Permanent Delete",e.removeEventListener("click",m),e.addEventListener("click",y,{once:!0}),setTimeout(()=>{e.textContent="Permanently Delete",e.removeEventListener("click",y),e.addEventListener("click",m)},3e3)}async function y(){var e,a;const t=o();if(t)try{await axios.delete(`/api/admin/shops/${t}/force`),window.toast.success("Shop permanently deleted!"),setTimeout(()=>window.location.href="/admin/shops",1500)}catch(s){console.error("Error deleting shop:",s),window.toast.error(((a=(e=s.response)==null?void 0:e.data)==null?void 0:a.message)||"Failed to delete shop")}}function v(t){if(!t)return"";const e=t.split(":");let a=parseInt(e[0]);const s=e[1],n=a>=12?"PM":"AM";return a=a>12?a-12:a==0?12:a,`${a}:${s} ${n}`}document.addEventListener("DOMContentLoaded",()=>{p(),b()});
